...
void ADC(void)						//Lite Demo of the Analog to Digital Converter
{
	...
	volts[0x3]=' ';
	volts[0x4]= 0;
	ADC_Get(); //set Hadc and Ladc
	Acc = (((int)HADC) * 0x100 + (int)(LADC))*0xA; //multiply high by 256 add to low times 10
	volts[0x0] = 48 + (Acc / 0x7FE); //getting ones place
	Acc = Acc % 0x7FE; //v-max = 2046
	volts[0x2] = ((Acc *0xA) / 0x7FE) + 48; //converting to ascii
	Acc = (Acc * 0xA) % 0x7FE;
	if (Acc >= 0x3FF) volts[0x2]++; //get tens place
	if (volts[0x2] == 58) //deals with ascii table, puts voltage into chars is tens place over the value of nine...
	{
		volts[0x2] = 48;
		volts[0x0]++;
	}
