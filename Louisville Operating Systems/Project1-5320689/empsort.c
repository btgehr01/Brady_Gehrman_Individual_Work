...
struct LList *createList()
{
    ...
    {
        // initialize LList properties to NULL (head and tail)
        list->head = NULL;
        list->tail = NULL;
    }
    return list;
...
void deleteList(struct LList *list)
{
    ...
    while (currentNode != NULL)
    {
        ...
        // free the first name data in memory
        free(currentNode->lastName);
        // free the last name in memory
        free(currentNode->employeeId);
        // free the employeeId in memory
        tmp = currentNode;
...
void insert_sorted(struct LList *list, struct Node *node)
{
    ...
    else
    {
        ...
        // start at the node at the head of the list
        int insertedNode = 0;
        while (!insertedNode)
        {
