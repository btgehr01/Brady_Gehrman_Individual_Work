...
namespace NWH.VehiclePhysics2.Input
{
    ...
    public class SteeringWheelInputProvider : VehicleInputProviderBase
    {
        ...
        /// <summary>
        /// If enabled gears will have to be continously held to keep them in.
        /// </summary>
        [UnityEngine.Tooltip("If enabled gears will have to be continously held to keep them in.")]
        public bool hShifterHoldForGear = false;
        ...
        void SetVehicleInputs()
        {
            ...
            // H-shifter
            _shiftIntoInput = -999;
            if (GetButtonDown(shiftIntoReverseButton) || (hShifterHoldForGear && GetButtonPressed(shiftIntoReverseButton)))
            {
                _shiftIntoInput = -1;
            }
            else if (GetButtonDown(shiftIntoNeutralButton) || (hShifterHoldForGear && GetButtonPressed(shiftIntoNeutralButton)))
            {
                Debug.Log("Hello");
                _shiftIntoInput = 0;
            }
            else if (GetButtonDown(shiftInto1stButton) || (hShifterHoldForGear && GetButtonPressed(shiftInto1stButton)))
            {
                _shiftIntoInput = 1;
            }
            else if (GetButtonDown(shiftInto2ndButton) || (hShifterHoldForGear && GetButtonPressed(shiftInto2ndButton)))
            {
                _shiftIntoInput = 2;
            }
            else if (GetButtonDown(shiftInto3rdButton) || (hShifterHoldForGear && GetButtonPressed(shiftInto3rdButton)))
            {
                if (hShifterUse34asDR)
                {
                    vehicleController.powertrain.transmission.ShiftInto(1, true);
                    _shiftIntoInput = -1;
                }
                else
                {
            ...
            else if (GetButtonDown(shiftInto4thButton) || (hShifterHoldForGear && GetButtonPressed(shiftInto4thButton)))
            {
                if (hShifterUse34asDR)
                {
                    vehicleController.powertrain.transmission.ShiftInto(-1, true);
            ...
            else if (GetButtonDown(shiftInto5thButton) || (hShifterHoldForGear && GetButtonPressed(shiftInto5thButton)))
            {
                _shiftIntoInput = 5;
            }
            else if (GetButtonDown(shiftInto6thButton) || (hShifterHoldForGear && GetButtonPressed(shiftInto6thButton)))
            {
                _shiftIntoInput = 6;
            }
            else if (GetButtonDown(shiftInto7thButton) || (hShifterHoldForGear && GetButtonPressed(shiftInto7thButton)))
            {
                _shiftIntoInput = 7;
            }
            else if (GetButtonDown(shiftInto8thButton) || (hShifterHoldForGear && GetButtonPressed(shiftInto8thButton)))
            {
                _shiftIntoInput = 8;
            }
            else if (GetButtonDown(shiftInto9thButton) || (hShifterHoldForGear && GetButtonPressed(shiftInto9thButton)))
            {
                _shiftIntoInput = 9;
            }
            else if (hShifterHoldForGear)
            {
                _shiftIntoInput = 0;
            }
