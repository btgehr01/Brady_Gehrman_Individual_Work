            ...
            if (!visualOnly && damageIntensity > 0)
            {
                ...
                foreach (WheelComponent wc in vc.Wheels)
                {
                    if (Vector3.Distance(collisionPoint, wc.wheelController.center) <
                        wc.wheelController.wheel.radius * 2.5f)
                    {
                        wc.Damage += damage;
                    }
                }
                ...
                if (Vector3.Distance(vc.WorldEnginePosition, collisionPoint) < dimensionsMagnitude * 0.3f)
                {
                    vc.powertrain.engine.ComponentDamage += damage;
                }

                if (Vector3.Distance(vc.WorldTransmissionPosition, collisionPoint) < dimensionsMagnitude * 0.3f)
                {
