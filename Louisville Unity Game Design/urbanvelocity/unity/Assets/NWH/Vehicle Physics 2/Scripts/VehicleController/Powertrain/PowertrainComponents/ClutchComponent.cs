...
namespace NWH.VehiclePhysics2.Powertrain
{
    ...
    public partial class ClutchComponent : PowertrainComponent
    {
        ...
        /// </summary>
        [UnityEngine.Tooltip("Transmission shift flag.")]
        public bool shiftSignal;

        /// <summary>
        /// Engine start flag.
        /// </summary>
        [UnityEngine.Tooltip("Engine start flag.")]
        ...
        public override float ForwardStep(float torque, float inertiaSum, float t, float dt, int i)
        {
            ...
            {
                return CalculateTorqueRealistic(torque, inertiaSum, t, dt, i);
            }
            else
            {
                return CalculateTorqueArcade(torque, inertiaSum, t, dt, i);
            }
        }


        private float CalculateTorqueArcade(float torque, float inertiaSum, float t, float dt, int i)
        {
            ...
            float returnTorque =
                outputA.ForwardStep(torque * (1f - (1f - Mathf.Pow(clutchEngagmentCurveValue, 0.3f))),
                                    inertiaSum * clutchEngagement + inertia, t, dt, i) * clutchEngagmentCurveValue;
        ...
        private float CalculateTorqueRealistic(float torque, float inertiaSum, float t, float dt, int i)
        {
            float clutchEngagmentCurveValue = clutchEngagementCurve.Evaluate(clutchEngagement);
